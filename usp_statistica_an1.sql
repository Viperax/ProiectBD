...
if exists (select * from sysobjects where id = object_id('dbo.usp_statistica_an1') and sysstat & 0xf = 4)
	drop procedure "dbo"."usp_statistica_an1"
GO

use Catalog
go
CREATE PROCEDURE usp_statistica_an1 @id_dep int , @id_specializare int,  @id_an int
AS
BEGIN
	...
	(SELECT GrupaID 
	FROM Grupa 
	WHERE An=1 AND AnScolarID=@id_an AND SpecializareID = @id_specializare ) ------------------
